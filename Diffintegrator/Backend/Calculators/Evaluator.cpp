...
class ExpressionEvaluator : public boost::static_visitor<double> {
public:
    
    ExpressionEvaluator(const std::shared_ptr<Math::Evaluator::GlobalContext> global_context,
                        const std::map<Math::Variable, double>&& local_variables)
...
//    double operator()(const Math::CallOperator& expr) noexcept(false) {
...
private:
    ...
    const std::map<Math::Variable, double> m_local_variables;
    
};

double Math::Evaluator::operator()(const ExpressionNode& node, const std::shared_ptr<Evaluator::GlobalContext> context) const {
    for (const NumberReference& ref : node.number_ref_dependencies()) {
        if (!context->number_references.contains(ref)) {
            ...
            double result = eval(dep_node, context);
